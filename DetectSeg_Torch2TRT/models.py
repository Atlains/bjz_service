...
from Yolov3_utils.utils import build_targets, to_cpu, non_max_suppression
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None, img_dim=None, Half=False):
        ...
        # print(x.shape)
        # print(num_samples, self.num_anchors, self.num_classes + 5, grid_size, grid_size)
        prediction = (
        ...
        # print(x.data.type(), self.grid_x.type())
...
class YOLOHead(nn.Module):
   ...
   Build a detection head.
    ...
    def forward(self, backbone_out, Half=False):

        if len(self.yolo_layer) == 2:
            x1, loss1 = self.yolo_layer[0](backbone_out[0], targets=None, img_dim=self.img_size, Half=Half)
            x2, loss2 = self.yolo_layer[1](backbone_out[1], targets=None, img_dim=self.img_size, Half=Half)
