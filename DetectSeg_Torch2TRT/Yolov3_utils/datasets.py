...
from Yolov3_utils.augmentations import horisontal_flip
...
class ImageFolder_SC(Dataset):
    ...
    def __getitem__(self, index):
        img_path = self.files[index % len(self.files)]
        # Extract image as PyTorch tensor
        orig_img = cv2.imread(img_path).copy()
        img_nd = torch.from_numpy(orig_img[:,:,0]).float()
