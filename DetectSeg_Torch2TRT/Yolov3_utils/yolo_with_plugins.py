...
class TrtYOLO(object):
    ...
    def detect1(self, img, conf_th=0.3, letter_box=None, batch_size=1):
        """Detect objects in the input image."""
        letter_box = self.letter_box if letter_box is None else letter_box
        img_resized = resize(img[None, ...], self.input_shape).numpy().astype(np.float32)
        img_resized = img_resized.astype(np.float32)

        img_resized = np.repeat(img_resized[None, ...], batch_size, axis=0)
        # img_resized = np.random.randn(16, 1, 416, 416).astype(np.float32)
        ...
        trt_outputs = self.inference_fn(
            ...
            inputs=self.inputs,
            outputs=self.outputs,
            stream=self.stream)     # shape: [[bs*2535*7]]
        if self.cuda_ctx:
            self.cuda_ctx.pop()

        trt_outputs = [[i] for i in trt_outputs[0].reshape((-1, 2535, 7))]

        reses = []
        for trt_output in trt_outputs:
            # print(trt_output[0].shape)
            boxes, scores, classes = _postprocess_yolo(
                trt_output, img.shape[1], img.shape[0], conf_th,
                nms_threshold=0.5, input_shape=self.input_shape,
                letter_box=letter_box)
            ...
            res = np.concatenate((boxes, scores, np.zeros_like(scores), classes), axis=1)
            reses.append(res)
        return reses


    def detect2(self, img, conf_th=0.3, letter_box=None, batch_size=1):
        """Detect objects in the input image."""
        letter_box = self.letter_box if letter_box is None else letter_box
        ...
        for trt_output in trt_outputs:
            ...
            boxes, scores, classes = _postprocess_yolo(
                trt_output, img.shape[-1], img.shape[-2], conf_th,
                nms_threshold=0.5, input_shape=self.input_shape,
                letter_box=False)

            # clip x1, y1, x2, y2 within original image
            boxes[:, [0, 2]] = np.clip(boxes[:, [0, 2]], 0, img.shape[-1]-1)
            boxes[:, [1, 3]] = np.clip(boxes[:, [1, 3]], 0, img.shape[-2]-1)
            scores = scores.reshape(-1, 1)
            classes = classes.reshape(-1, 1)
            res = np.concatenate((boxes, scores, np.zeros_like(scores), classes), axis=1)
            reses.append(res)
